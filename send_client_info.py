    ...
    try:
        ...
        with MSSQL_DATABASE_CONNECTION:
            ...
            for row in MSSQL_DATABASE_CURSOR.fetchall():
                ...
                if not row[5] and isinstance(row[5], datetime.date):
                ...
                if not row[9 and isinstance(row[9], datetime.date):
                    ...
                    first_call = ''
                if not row[10] and isinstance(row[10], datetime.date):
                    last_call = row[10].strftime('%Y-%m-%d')
                else:
                    last_call = ''
                if not row[11] and isinstance(row[11], datetime.date):
                    first_meeting = row[11].strftime('%Y-%m-%d')
                else:
                    first_meeting = ''
                if not row[12] and isinstance(row[12], datetime.date):
